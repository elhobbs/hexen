...
// look up sector number for each subsector
	waitforit3(__FILE__":"STRINGIZE(__LINE__));
	ss = subsectors;
...
void P_Init(void)
{
	waitforit("InitMapInfo");
	InitMapInfo();
	waitforit("P_InitSwitchList");
	P_InitSwitchList();
	waitforit("P_InitFTAnims");
	P_InitFTAnims(); // Init flat and texture animations
	waitforit("P_InitTerrainTypes");
	P_InitTerrainTypes();
	waitforit("P_InitLava");
	P_InitLava();
	//printf("%08x\n", sprnames);
	waitforit("R_InitSprites");
